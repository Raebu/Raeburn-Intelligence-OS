...
from .predictive_api import router as predictive_router
...
from .service import company_index, digital_twin, discover_and_refresh, discover_companies, opportunity_feed, refresh_company, resolve_company_number, system_status
...
async def protect_operator_actions(request: Request, call_next):
    if request.method in {"POST", "PUT", "PATCH", "DELETE"}:
        try: validate_operator_key(request.headers.get("X-API-Key"))
...
def dashboard() -> HTMLResponse: return HTMLResponse(DASHBOARD_HTML)
@app.get("/health")
def health() -> dict[str, str]: return {"status": "ok", "version": __version__}
@app.get("/v1/status")
def status() -> dict: return system_status()
@app.get("/v1/sources", response_model=list[SourceRecord])
def list_sources() -> list[SourceRecord]: return get_sources()
...
def companies(limit: int = Query(default=100, ge=1, le=1000)) -> list[dict]: return company_index(limit=limit)
...
def discovery_search(q: str = Query(min_length=2, max_length=200), limit: int = Query(default=20, ge=1, le=100)) -> list[dict]:
    try: return discover_companies(q, limit=limit)
    except ExternalServiceError as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def discovery_refresh(q: str = Query(min_length=2, max_length=200), limit: int = Query(default=10, ge=1, le=50)) -> dict:
    try: return discover_and_refresh(q, limit=limit)
    except ExternalServiceError as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
@app.post("/v1/opportunities/score", response_model=Opportunity)
def score(request: OpportunityScoreRequest) -> Opportunity: return score_opportunity(request)
@app.post("/v1/opportunities/score-all", response_model=list[Opportunity])
def score_every_kind(request: OpportunityScoreRequest) -> list[Opportunity]: return score_all(request)
@app.post("/v1/companies/{company_number}/refresh")
def refresh(company_number: str) -> dict:
    try: return refresh_company(company_number).model_dump()
    except KeyError as exc: raise HTTPException(status_code=404, detail="Company not found") from exc
    except ExternalServiceError as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
@app.post("/v1/companies/{company_id}/enrich/companies-house")
def companies_house_enrichment(company_id: str) -> dict:
    try: return enrich_companies_house(company_id)
    except KeyError as exc: raise HTTPException(status_code=404, detail="Company not found") from exc
    except ExternalServiceError as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def technology_enrichment(company_id: str, url: str = Query(min_length=8, max_length=2048)) -> dict:
    try: return enrich_technology(company_id, url)
    except KeyError as exc: raise HTTPException(status_code=404, detail="Company not found") from exc
    except (ExternalServiceError, ValueError) as exc: raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def find_a_tender(updated_from: str | None = None, updated_to: str | None = None, stages: str | None = None, limit: int = Query(default=100, ge=1, le=100), cursor: str | None = None) -> dict:
    try: return FindATenderClient().releases(updated_from=updated_from, updated_to=updated_to, stages=stages, limit=limit, cursor=cursor)
    except ExternalServiceError as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def contracts_finder(published_from: str | None = None, published_to: str | None = None, stage: list[str] | None = None, size: int = Query(default=100, ge=1, le=100), page: int = Query(default=1, ge=1)) -> dict:
    try: return ContractsFinderClient().search(published_from=published_from, published_to=published_to, stages=stage, size=size, page=page)
    except ExternalServiceError as exc: raise HTTPException(status_code=503, detail=str(exc)) from exc
...
def company_by_number(company_number: str) -> dict:
    ...
    if company is None: raise HTTPException(status_code=404, detail="Company not found in local intelligence store")
    return company.model_dump()
@app.get("/v1/companies/{company_id}/twin")
def read_digital_twin(company_id: str) -> dict:
    try: return digital_twin(company_id)
    except KeyError as exc: raise HTTPException(status_code=404, detail="Company not found") from exc
@app.get("/v1/opportunities")
def read_opportunity_feed(limit: int = Query(default=100, ge=1, le=1000)) -> list[dict]: return opportunity_feed(limit=limit)
@app.get("/v1/opportunities.csv")
def export_opportunities(limit: int = Query(default=1000, ge=1, le=10000)) -> StreamingResponse:
    output = io.StringIO(); writer = csv.writer(output); writer.writerow(["score", "confidence", "type", "company", "company_number", "recommended_action"])
