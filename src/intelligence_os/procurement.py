    ...
    if isinstance(packages, list):
        ...
        for package in packages:
            if isinstance(package, dict):
                rows.extend(
                    row for row in package.get("releases", []) if isinstance(row, dict)
                )
        return rows
    return []
...
def _company_number_from_supplier(supplier: dict[str, Any]) -> str | None:
    ...
    candidates = [
        supplier.get("company_number"),
        identifier.get("id") if isinstance(identifier, dict) else None,
    ]
...
def attach_awards_to_indexed_companies(records: list[dict[str, Any]]) -> dict[str, int]:
    linked = 0
    awards = 0
    for record in records:
        for supplier in record.get("suppliers", []):
            ...
            linked += 1
            awards += 1
    return {"records": len(records), "linked_companies": linked, "awards_attached": awards}
